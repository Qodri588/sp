...
function createMockAIEngine() {
  return {
    generateInitial: mock(() =>
      Promise.resolve({ text: 'Generated prompt', title: 'Title', lyrics: 'Lyrics' })
    ),
    refinePrompt: mock(() =>
      Promise.resolve({ text: 'Refined prompt', title: 'Title', lyrics: 'Lyrics' })
    ),
...
describe('RPC Handlers', () => {
  ...
  describe('quick vibes handlers', () => {
    test('generateQuickVibes returns prompt', async () => {
      ...
      expect(result.versionId).toBeDefined();
      expect(aiEngine.generateQuickVibes).toHaveBeenCalledWith(
        'lofi-study',
        'relaxing',
        [],
        expect.anything()
...
bpm: "110"`;
  ...
  describe('validation error handling', () => {
    ...
    test('generateCreativeBoost throws ValidationError for invalid creativityLevel', async () => {
      ...
      await expect(
        handlers.generateCreativeBoost({
