...
function RequestParams({
  ...
  request: TraceLLMCallEvent['request'];
}): ReactElement | null {
  const items = [
    request.temperature !== undefined && (
      <span key="t">
        <span className="text-muted-foreground">temp:</span> {request.temperature}
      </span>
    ),
    request.maxTokens !== undefined && (
      <span key="m">
        <span className="text-muted-foreground">maxTokens:</span> {request.maxTokens}
      </span>
    ),
    request.maxRetries !== undefined && (
      <span key="r">
        <span className="text-muted-foreground">maxRetries:</span> {request.maxRetries}
      </span>
    ),
  ].filter(Boolean);
  return items.length > 0 ? (
    <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs">{items}</div>
  ) : null;
}
...
function TelemetrySection({
  ...
  telemetry: TraceLLMCallEvent['telemetry'];
}): ReactElement | null {
  ...
  return (
    <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs border-t pt-2">
      ...
      {telemetry.tokensIn !== undefined && (
        <span>
          <span className="text-muted-foreground">in:</span> {telemetry.tokensIn}
        </span>
      )}
      {telemetry.tokensOut !== undefined && (
        <span>
          <span className="text-muted-foreground">out:</span> {telemetry.tokensOut}
        </span>
      )}
      {telemetry.finishReason && (
        <span>
          <span className="text-muted-foreground">reason:</span> {telemetry.finishReason}
        </span>
      )}
    </div>
  );
...
function AttemptsSection({
  ...
  attempts: TraceLLMCallEvent['attempts'];
}): ReactElement | null {
      ...
      {attempts.map((attempt, idx) => (
        <div
          ...
          className={cn(
            ...
            attempt.error ? 'border-destructive/50 bg-destructive/5' : 'border-border'
          )}
        >
          <div className="flex gap-2">
            <span>#{attempt.attempt}</span>
            <span className="text-muted-foreground">{formatLatency(attempt.latencyMs)}</span>
          </div>
          {attempt.error && (
            <div className="text-destructive mt-1">
              [{attempt.error.type}] {attempt.error.message}
              {attempt.error.status && ` (${attempt.error.status})`}
            </div>
          )}
        </div>
      ))}
...
function AdvancedSection({
  ...
  request: TraceLLMCallEvent['request'];
  response: TraceLLMCallEvent['response'];
}): ReactElement | null {
      ...
      <CollapsibleContent className="space-y-3 pt-2">
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-between">
              <span className="text-xs text-muted-foreground">
                Raw messages ({request.messages.length})
              </span>
        ...
        {response.rawText && (
          <div className="space-y-1">
            ...
            <div className="text-xs bg-muted/50 rounded p-2 font-mono whitespace-pre-wrap break-words max-h-40 overflow-y-auto">
              {response.rawText}
            </div>
          </div>
        )}
    ...
    <div className="rounded-lg border bg-card/50 p-3 space-y-3">
      ...
      <div className="space-y-1">
        <div className="text-xs text-muted-foreground flex gap-2">
          <span>Input: {request.inputSummary.messageCount} messages</span>
          <span>•</span>
          <span>{request.inputSummary.totalChars.toLocaleString()} chars</span>
        </div>
        <div className="text-xs bg-muted/50 rounded p-2 font-mono whitespace-pre-wrap break-words max-h-20 overflow-hidden">
          {request.inputSummary.preview}
        </div>
      </div>
      <div className="space-y-1">
