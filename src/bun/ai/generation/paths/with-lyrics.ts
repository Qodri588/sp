...
async function runGenreDetection(
  ...
  return detectGenreFromTopic(lyricsTopic.trim(), getModelFn, undefined, {
    trace,
...
export async function generateWithLyrics(
  ...
  const [thematicContext, genreResult] = await Promise.all([
    ...
    runGenreDetection(willDetectFromTopic, lyricsTopic, getModelFn, trace),
  ]);
  ...
  logGenreResolution(descriptionGenre, genreResult?.genre, description, lyricsTopic, false);
